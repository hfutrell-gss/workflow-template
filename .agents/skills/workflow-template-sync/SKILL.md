...
2. Clears template-only identity: example content out of `journal/` and `playbooks/`, any
   INSTANCE run state under `workflows/<workflow>/<target>/` (a bare `workflows/<workflow>/`
   with no target subdirs is the DURABLE procedure and survives), and any legacy run state
   under `.workflow/`. That matters because both are **committed** (unlike `workspace/`),
   so a derive-by-clone would otherwise carry the template's own in-flight runs and notes
   into the new workflow. A run belongs to the repo that performed it.
