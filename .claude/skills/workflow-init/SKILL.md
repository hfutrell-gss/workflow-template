...
match `.claude/skills/workflow-init/VERSION`. This skill makes that true.

## Run
```sh
.claude/skills/workflow-init/init.sh --check   # verify only (what the mandate requires)
.claude/skills/workflow-init/init.sh           # install anything missing + write init.lock
```
...
- **codegraph** — defaults to the official install method from
  ...
  specifically. Override with `CODEGRAPH_INSTALL="<command>" .claude/skills/workflow-init/init.sh`
